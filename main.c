...
container_start ()
{
  ...
  if ((container_pid =
       container_init (&config, stack + CONTAINER_STACK_SIZE)) == -1)
    {
  ...
  if (user_namespace_prepare_mappings (container_pid, sockets[0]))
    {
      exitcode = 1;
      printf
        ("failed to user_namespace_set_user: %s\nstopping container...\n",
         strerror (errno));
      free (stack);
      return exitcode;
    }
  ...
  if (sig == SIGINT || sig == SIGTERM)
    {
      ...
      if (child_pid > 0)
        {
          logdoc (LOG_LVL_INFO, LOGFILE, "Sending SIGINT to child %d",
                  child_pid);
      ...
      if (container_pid > 0)
        {
          logdoc (LOG_LVL_INFO, LOGFILE, "Sending SIGINT to container %d",
                  container_pid);
...
main (int argc, char *argv[])
{
  ...
  while ((value =
          getopt_long (argc, argv, "c:m:l:p:dh", long_options,
                       &option_index)) != -1)
    {
      switch (value)
        {
  ...
  if (start_daemonized == 1)
    {
      ...
      daemonize ();
      logdoc (LOG_LVL_WARNING, LOGFILE, "daemonize() [OK]");
