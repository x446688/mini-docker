...
user_namespace_init(uid_t uid, int fd) {
  ...
  logdoc("INFO",LOGFILE,"setting user namespace...");
  
  logdoc("INFO",LOGFILE,"writing to socket...");
  if (write(fd, &unshared, sizeof(unshared)) != sizeof(unshared)) {
    logdoc("INFO",LOGFILE,"failed to write socket");
    return -1;
  }

  logdoc("INFO",LOGFILE,"reading from socket...");
  if (read(fd, &result, sizeof(result)) != sizeof(result)) {
    logdoc("INFO",LOGFILE,"failed to read from socket");
    return -1;
  }
  ...
  logdoc("INFO",LOGFILE,"switching to uid / gid...");

  logdoc("INFO",LOGFILE,"setting uid and gid mappings...");
  if (setgroups(1, &(gid_t){uid}) == -1 && errno != EPERM) {
    logdoc("ERROR",LOGFILE,"setgroups failed");
    return -1;
  }
  if (setresgid(uid, uid, uid) || setresuid(uid, uid, uid)) {
    logdoc("ERROR",LOGFILE,"failed to set uid / gid mappings");
    return -1;
  }

  logdoc("INFO",LOGFILE,"user namespace set");
...
__set_capabilites () {
  ...
  caps = cap_get_proc();
  if (!caps) {
    logdoc("ERROR", LOGFILE, "cap_get_proc failed");
    return -1;
  }
  ...
  cap_set_flag(caps, CAP_INHERITABLE, sizeof(drop_caps)/sizeof(int), drop_caps, CAP_CLEAR)) {
    logdoc("ERROR", LOGFILE, "cap_set_flag failed");
    cap_free(caps);
    return -1;
  }
  if (cap_set_proc(caps) == -1) {
      logdoc("ERROR", LOGFILE, "cap_set_proc failed");
      cap_free(caps);
      return -1;
  }
...
__on_start (void *arg)
{
  ...
  logdoc("ERROR", LOGFILE, "DEBUG hostname bytes:");
  for (int i = 0; i < 4; i++) {
    char buf[64];
    ...
    logdoc("ERROR", LOGFILE, buf);
  }
  logdoc("ERROR",LOGFILE,config->hostname);
  if (sethostname ("mini", 4))
    {
      logdoc("ERROR",LOGFILE,"sethostname failed");
      logdoc("ERROR",LOGFILE,strerror(errno));	
      close (config->fd);
      return -1;
    }
  if (mount_set (config->mnt) ) {
      logdoc("ERROR",LOGFILE,"mount failed");
      logdoc("ERROR",LOGFILE,strerror(errno));
      close (config->fd);
      return -1;
    }
  if (user_namespace_init (config->uid, config->fd)) {
      logdoc("ERROR",LOGFILE,"failed to init user namespace");
      logdoc("ERROR",LOGFILE,strerror(errno));
      close (config->fd);
      return -1;
  }
  if (__set_capabilites ()) {
      logdoc("ERROR",LOGFILE,"failed to set caps");
      logdoc("ERROR",LOGFILE,strerror(errno));
      close (config->fd);
      return -1;
  }
  if (close (config->fd))
    {
      logdoc("ERROR",LOGFILE,"closing container socket failed");
      logdoc("ERROR",LOGFILE,strerror(errno));
      return -1;
    }
  ...
  if (execve (config->cmd, argv, NULL) == -1)
    {
      logdoc("ERROR",LOGFILE,"failed to execve");
      return -1;
    }
  return 0;
}
...
cgroups_init(char *hostname, pid_t pid) {
  ...
  cgroups_setting cgroups_settings[] = {
    ...
    *procs_setting};
  logdoc("WARN",LOGFILE,"initializing cgroup_dir...");
